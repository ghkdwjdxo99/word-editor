...
- 장기 반복 검증의 상세 결과는 `docs/test-reports/vX.Y.Z/VXXX-PNN[-PNN]_검증_YYYY-MM-DD.md` 형식으로 둔다. 같은 날 재실행본이 여러 개면 파일명 끝에 `_01`, `_02`를 붙인다.
- 통합 보고서에는 상세 보고서 경로와 최종 판정만 요약하고, 환경·절차·증거·결함 재현은 버전별 상세 보고서에 누적한다.
- 버전과 패치 상태는 `docs/project/버전_이력.md`에 누적한다.
- 테스트 결함과 수정·재검증 상태는 `docs/project/결함_및_패치_이력.md`에 누적한다.
- 역할, 승인 절차, 인수인계 규칙은 `docs/workflow/`에 둔다.
- 역할별 작업 요청은 `docs/workflow/handoffs/vX.Y.Z/`에 둔다.
- 배포 정책은 `docs/project/배포_정책.md`에 둔다.
- 장기 기능 순서는 `docs/project/제품_로드맵.md`에 둔다.
...
3. 개발자가 패치 ID 단위로 구현하고 버전 이력을 갱신한다.
   결함 수정이면 `docs/project/결함_및_패치_이력.md`도 함께 갱신한다.
